...
import AdminChatListPane from "./AdminChatListPane";
...
export default function AdminChatWindow() {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setMessages(msgs);
        if (conv) {
          await markAsRead(conversationId, adminId);
        }
      } catch (error) {
    ...
    const unsubscribe = subscribeToMessages(conversationId, (payload) => {
      if (payload.eventType === "INSERT") {
        ...
        markAsRead(conversationId, adminId);
  ...
  const otherParticipant = conversation?.participants.find(p => p.role !== "admin");
  const displayName = otherParticipant?.profile?.display_name || otherParticipant?.user_id || "User";
      ...
      <ChatLayout sidebar={<AdminChatListPane currentConversationId={undefined} />}>
        <div className="flex-1 flex items-center justify-center bg-slate-50/50 dark:bg-slate-900/50">
          <div className="text-center space-y-4">
            <div className="bg-blue-100 dark:bg-blue-900/20 p-6 rounded-full inline-flex mb-2 shadow-sm">
              <Send className="h-10 w-10 text-blue-600 dark:text-blue-400" />
            </div>
            <h3 className="text-xl font-semibold text-slate-900 dark:text-white">Admin Messages</h3>
  ...
  return (
    <ChatLayout sidebar={<AdminChatListPane currentConversationId={conversationId} />}>
      <Seo
        title={`Chat with ${displayName} – Admin`}
      ...
      <ChatHeader>
        <div className="flex items-center gap-4">
          <div className="md:hidden">
            <Button variant="ghost" size="icon" onClick={() => navigate("/admin/chats")} className="text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white">
