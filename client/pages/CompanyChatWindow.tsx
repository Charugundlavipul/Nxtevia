...
export default function CompanyChatWindow() {
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      try {
        // Parallelize fetching conversation and messages
        // Default limit is 50, which helps initial load
        ...
        if (conv) {
          // Verify we aren't marking read needlessly if already read, but this is fine for now
          await markAsRead(conversationId, companyId);
        }
      } catch (error) {
    ...
    const unsubscribe = subscribeToMessages(conversationId, (payload) => {
      if (payload.eventType === "INSERT") {
        ...
        markAsRead(conversationId, companyId);
  ...
  // Ensure we get the *other* participant, not just seeker (could be admin)
  const otherParticipant = conversation?.participants.find(p => p.role !== "company");
  const displayName = otherParticipant?.profile?.display_name || (otherParticipant?.role === "admin" ? "Admin" : otherParticipant?.user_id) || "Seeker";
  const initials = displayName?.split(" ").map((n: string) => n[0]).join("").toUpperCase() || "S";
  ...
  // Safety check, though routing handles this
  if (!conversationId) return null;

  return (
    <>
      <Seo
        title={`Chat with ${displayName} – Company`}
      ...
      <ChatHeader>
        <div className="flex items-center gap-4">
          <div className="md:hidden">
            <Button variant="ghost" size="icon" onClick={() => navigate("/company/chats")} className="text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white">
      ...
      <ChatInputArea>
        ...
        <div className="flex items-end gap-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-2 focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all shadow-sm">
          ...
          <Button
            ...
            size="icon"
            className="h-9 w-9 rounded-xl shrink-0 mb-0.5 transition-all bg-primary hover:bg-primary/90 text-white shadow-sm disabled:opacity-50 disabled:cursor-not-allowed"
          >
